...
__m256 nv_sin_8(__m256 x) {
...

	{
		...
		const auto X{ c4.z * r2_x };
		const auto Z{ c4.z * r2_z };
		const auto Y{ c4.w * r2_y };

		r2_y = Z + Y + X;// range check: 0.25 to 0.75
...
__m256 nv_acos_8(__m256 x) {
	...
	return _mm256_fmadd_ps(negate, _mm256_set1_ps(float(M_PI)), y);
}
...
__m256 intel_exp_8(__m256 x) {
	...
	fx = flr - _mm256_and_ps(_mm256_cmp_ps(flr, fx, _CMP_GT_OS), MM256_SET1(1.f));	
